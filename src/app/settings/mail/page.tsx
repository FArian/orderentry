...
interface ProviderGuide {
  ...
  vars: (authType: MailAuthType) => EnvVar[];
  notes?: string;
}
...
const PROVIDER_GUIDES: Record<MailProvider, ProviderGuide> = {
  smtp: {
    ...
    vars: (auth) => [
      ...
      { name: "MAIL_HOST",      example: "smtp.example.com",  secret: false },
  ...
  smtp_oauth2: {
    ...
    vars: (_auth) => [
      ...
      { name: "MAIL_HOST",                example: "smtp.office365.com",  secret: false },
      { name: "MAIL_PORT",                example: "587",                 secret: false, optional: true },
...
export default function MailConfigPage() {
  ...
  }, [selectedProvider, allowedAuth, selectedAuth]);
    ...
    <div className="flex min-h-screen bg-zt-bg-page">
      ...
      <main className="flex-1 p-8 max-w-3xl">
        ...
        <Card title={t("mail.reference")} className="mb-6">
          ...
          {/* Notes for selected provider */}
          {guide.notes && (
            <div className="mb-4 p-3 bg-zt-info-light border border-zt-info-border rounded text-[12px] text-zt-info">
              {guide.notes}
            </div>
          )}
          ...
          <div className="rounded border border-zt-border overflow-hidden">
            {guide.vars(selectedAuth).map((v) => <EnvVarRow key={v.name} {...v} />)}
          </div>
        </Card>
